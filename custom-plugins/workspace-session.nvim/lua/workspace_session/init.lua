...
local function source_buffers(source, roots)
  local result = {}
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if normal_file_buffer(buf) then
      local owned = M.owner(vim.api.nvim_buf_get_name(buf), roots)
...
local function map_first_visit(source, destination, roots, views)
  ...
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    ...
    if normal_file_buffer(buf) then
      ...
      local owned = M.owner(path, roots)
...
function M.switch(sessions, destination, roots, opts)
  ...
  local selected
  for _, root in ipairs(roots or {}) do
    if destination and destination.path and same_path(root.path, destination.path) then
      selected = root
      break
    end
  end
  ...
  if source_script then
    local rolled_back, rollback_error = restore(source_script)
